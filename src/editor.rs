...
impl Editor {
    pub fn default() -> Self {
        ...
        let mut initial_status = String::from("HELP: Ctrl-Q = quit | Ctrl-S = save | Ctrl-F = search");
        ...
        match pressed_key {
            Key::Ctrl('q') => {
                if self.quit_times > 0 && self.document.is_dirty() {
                    self.status_message = StatusMessage::from("Warning!! File has unsaved changes. Press Ctrl-Q again to quit.".to_string());
                    self.quit_times -= 1;
                    return Ok(());
                }
    ...
    fn draw_welcome_message(&self) {
        let mut welcome_message = format!("Rust Editor -- version {}\r", VERSION);
        let width = self.terminal.size().width as usize;
    ...
    fn search(&mut self) {
        let original_cursor_position = self.cursor_position.clone();
